import { getFaceDescriptorFromBase64, parseDescriptor, compareFaces, detectFaceDirectly } from './faceApiUtils';
...
export const mockApi = {
  ...
  verifyFacultyBiometricLogin: async (id, faceImage) => {
    ...
    if (faceImage && faceImage.startsWith('data:image')) {
      ...
      // ULTRA-LENIENT MATCH: Increased from 0.75 to 0.85 for "kuch bhi same mil jaye" reliability
      if (distance !== null && distance < 0.85) {
        const confidence = Math.max(0, 1 - (distance / 0.85)); // Normalize confidence to the new threshold
        return {
          success: true,
          faculty,
  ...
  verifyStudentBiometricLogin: async (id, faceImage) => {
    ...
    if (faceImage && faceImage.startsWith('data:image')) {
      ...
      // ULTRA-LENIENT MATCH: Increased from 0.75 to 0.85 for "kuch bhi same mil jaye" reliability
      if (distance !== null && distance < 0.85) {
        const confidence = Math.max(0, 1 - (distance / 0.85));
        return {
          success: true,
          student,
          confidence: confidence,
  ...
  matchFaceAcrossAllStudents: async (capturedImage) => {
    ...
    let bestMatch = null;
    let minDistance = 0.70; // Even more lenient matching (Elite Mode)
  ...
  matchFaceAcrossAllFaculty: async (capturedImage) => {
    ...
    const liveDescriptor = new Float32Array(detection.descriptor);
    let bestMatch = null;
    let minDistance = 0.70; // Elite Lenient Mode
